...
"""
Stage-1 Sparse RAE training script with pre-trained SOCAE quantizer.

This script trains the decoder of a sparse RAE (rae_s) with a frozen pre-trained
SOCAE quantizer. The training flow is:
  x -> encode (frozen) -> z -> quantize (frozen, pre-trained) -> z_sparse -> decode (trained) -> x_rec

The SOCAE quantizer should be pre-trained using train_socae.py before running this script.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train Stage-1 RAE with GAN and LPIPS losses."
    )
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="YAML config containing a stage_1 section.",
    )
    parser.add_argument(
        "--data-path",
        type=Path,
    ...
    parser.add_argument(
        "--wandb", action="store_true", help="Use Weights & Biases for logging if set."
    )
    parser.add_argument(
        "--compile",
        action="store_true",
        help="Use torch compile (for rae.encode, rae.forward).",
    )
...
def save_checkpoint(
    ...
    state = {
        ...
        "disc_scheduler": (
            disc_scheduler.state_dict() if disc_scheduler is not None else None
        ),
    }
...
def main():
    ...
    gan_cfg = (
        OmegaConf.to_container(gan_section, resolve=True)
        if gan_section is not None
        else {}
    )
    if not gan_cfg:
        raise ValueError(
            "Config must define a top-level 'gan' section for stage-1 training."
        )
    ...
    if eval_section:
        ...
        assert (
            len(eval_metrics) > 0
        ), "eval.metrics must contain at least one metric to compute."
    else:
        do_eval = False
    ...
    # Freeze the pre-trained quantizer if it exists
    if rae.quantizer is not None:
        rae.quantizer.eval()
        rae.quantizer.requires_grad_(False)
        quantizer_params = sum(p.numel() for p in rae.quantizer.parameters())
        if rank == 0:
            print(
                f"Freezing pre-trained SOCAE quantizer with {quantizer_params/1e6:.2f}M parameters"
            )

    ema_model = deepcopy(rae).to(device).eval()
    ema_model.requires_grad_(False)
    # only train decoder (encoder and quantizer are frozen)
    ...
    if disc_cfg.get("scheduler"):
        disc_scheduler, disc_sched_msg = build_scheduler(
            disc_optimizer, steps_per_epoch, disc_cfg
        )
    ...
    if rank == 0:
        ...
        logger.info(
            f"Using {disc_loss_type} discriminator loss and {gen_loss_type} generator loss."
        )
        logger.info(
            f"Perceptual (LPIPS) weight: {perceptual_weight:.6f}, GAN weight: {disc_weight:.6f}"
        )
        logger.info(
        ...
        print(
            disc_sched_msg if disc_sched_msg else "No LR scheduler for discriminator."
        )
        logger.info(
        ...
        for step, (images, _) in enumerate(loader):
            ...
            if train_disc:
                ...
                for _ in range(disc_updates):
                    ...
                    with autocast(**autocast_kwargs):
                        ...
                        fake_detached = (
                            torch.round((fake_detached + 1.0) * 127.5) / 127.5 - 1.0
                        )
                        ...
                        accuracy = (
                            (logits_real > logits_fake).float().mean()
            ...
            if global_step % sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    comparison = (
                        torch.cat([sample_images, samples], dim=0).cpu().float()
                    )
