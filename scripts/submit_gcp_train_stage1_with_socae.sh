...
case "$MODE" in
    train_stage1)
        ...
        ;;
    train_rae_s)
        SCRIPT="src/train_stage1_with_socae.py --config $CONFIG --data-path $DATA_PATH --results-dir $RESULTS_DIR/stage1_sparse --image-size $IMAGE_SIZE --precision $PRECISION $OPTIONAL_ARGS"
        ;;
    train_stage2)
